...
I plan on creating a choropleth map in Datawrapper/Flourish to show the changes in vaccine rates over time for infants and teenagers, respectively. I have initial graphics using ggplot, but because my angle changed in the middle of the day Wednesday after my correlation coefficient analysis resulted in little correlation, I was unfortunately unable to complete a polished graphic that I can embed, but plan on doing so tomorrow.
...
<h6> Old portion of pitch, outdated</h6>
...
### The 431XXXX series are administered to children 19-35 months, while the 132XXXX series are for adolescents aged 13-17 years. Let's do an unweighted mean of vaccination rates by the age range of the vaccine series instead of aggregating all four.
...
pct_change_agg <- report_card_data_aggregated %>% 
  group_by(cntyname) %>% 
  pivot_wider(
    names_from = year,
    values_from = mean_vax_rate,
...
# Percent change over 10 years for aggregated vaccines (NOT USING!) and for election difference.
pct_change_vax_and_election <- pct_change_agg %>% inner_join(
  election_pct_change, by=c("cntyname" = "cntyname")
) %>% 
  select(
    cntyname, pct_change_election_points, pct_change_2014_2024
  )
  
# Correlation between percent change in agg vaccine rates and percent change in election difference.
cor(pct_change_vax_and_election$pct_change_2014_2024, pct_change_vax_and_election$pct_change_election_points)
# There is no significant correlation between the percentage change in aggregated vaccination rates and the percentage change in the percentage point difference between the 2016 election and the 2024 election. I can't confidently compare the vaccination rates in a given county to how blue or red it voted in a given election. 
...
# Immunization status of children by school: 19-35 months.
...
<p>Thrishika Bala, epidemiologist for the state of Michigan<p>
